...
def refusal_score(
    ...
    precision_dtype = get_high_precision_dtype(logits.device)
    logits = logits.to(precision_dtype)
...
def select_direction(
    ...
    precision_dtype = get_high_precision_dtype(model_base.model.device)

    ablation_kl_div_scores = torch.zeros(
        (n_pos, n_layer), device=model_base.model.device, dtype=precision_dtype
    )
...
def kl_div_fn(
    ...
    """
    precision_dtype = get_high_precision_dtype(logits_a.device)
    logits_a = logits_a.to(precision_dtype)
    logits_b = logits_b.to(precision_dtype)
