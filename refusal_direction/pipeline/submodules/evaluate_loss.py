...
def compute_loss_over_dataset(
    ...
    precision_dtype = get_high_precision_dtype(model.device)
    accumulated_loss = torch.tensor(0, dtype=precision_dtype, device=model.device)
...
def evaluate_loss(
    ...
    for label in dataset_labels:
        if label == "pile":
            ...
            assert (
                completions_file_path is not None
